...
void AudioManager::playSoundOnce(string key)
{
	...
	if (SoundMap[key]->getStatus() != sf::Sound::Status::Playing)	//condition to make sure sounds arent forced to restart
	{
		SoundMap[key]->play();
	}
...
{
	if (SoundMap[key]->getStatus() == sf::Sound::Status::Playing)
	{
		SoundMap[key]->stop();
	}
}
...
{
	return (SoundMap[key]->getStatus() == sf::Sound::Status::Playing);
}
